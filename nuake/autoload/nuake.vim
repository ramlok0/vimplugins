...
function! s:OpenWindow() abort "{{{2
    ...
    if l:nuake_buf_nr != -1
        ...
        if g:nuake_per_buf == 1
            let l:curTermBuf = bufnr('%')
            let g:nuake_list[l:curBuf] = l:curTermBuf
            echom "set cur buf " . l:curBuf . " nuakeT " . "curTermBuf " .l:curTermBuf
        elseif g:nuake_per_tab == 1
            let l:curTermBuf = bufnr('%')
            " let g:nuake_list[tabpagenr()] = l:curTermBuf
            let t:nuake_buf_nr = l:curTermBuf
            echom "set cur tab " . t:nuake_buf_nr . " nuakeT " . "curTermBuf " .l:curTermBuf
        else
            let g:nuake_list[0] = l:curTermBuf
        endif
        call s:InitWindow()
        " call s:NuakeBufNr()
    endif
    " if has('nvim')
        " echom "NNVVIIMM"
        " let l:curTermBuf = bufnr('%')
        " let l:jobid = getbufvar(l:curTermBuf,'terminal_job_id')
        " " echom "job>>" . l:jobid . "<<"
        " call jobsend(l:jobid, "cd " . l:curDir . "\n")
        " call jobsend(l:jobid, "pwd\n")
    " endif
    " if has('vim')
        " echom "VVIIMM"
        " call term_sendkeys(l:nuake_buf_nr, "cd " . l:curDir . "\n")
        " call term_sendkeys(l:nuake_buf_nr, "pwd\n")
    " endif
    " echom "term >" . l:nuake_buf_nr . "<"
    " echom "curb$> " . bufnr('%') . "<"
...
" function! s:NuakeBufNr() abort "{{{2
    " if g:nuake_per_tab == 0 && g:nuake_per_buf == 0
    ...
    " else
        " if &filetype == 'nuake'
            " return bufnr('%')
        " else
            " if has_key(g:nuake_list,bufnr('%'))
                " " echom "return " . g:nuake_list[bufnr('%')]
                " return g:nuake_list[bufnr('%')]
            " else
               " return -1
            " endif
        " endif
...
function! s:NuakeBufNr() abort "{{{2
    if g:nuake_per_tab == 0 && g:nuake_per_buf == 0
        if empty(g:nuake_list)
            return -1
        elseif &filetype == 'nuake'
            return bufnr('%')
        endif
        return g:nuake_list[0]
    elseif g:nuake_per_tab == 1
        if ! exists(t:nuake_buf_nr)
            return -1
        elseif &filetype == 'nuake'
...
augroup nuake_tab_close
    if g:nuake_per_tab == 1
        autocmd!
        autocmd TabLeave * let s:temp_nuake_buf_nr = g: bufnr(s:NuakeBufNr())
        autocmd TabClosed * call s:NuakeCloseTab()
    endif
augroup END

function! s:NuakeOut() abort "{{{2
    let l:ccc = expand("<abuf>")
    if has_key(g:nuake_list,l:ccc)
        if bufexists( g:nuake_list[bufnr('%')] ) != -1
            let l:nn = g:nuake_list[bufnr('%')]
            echom "Delete buffer " . l:ccc . " term " .  l:nn
            execute 'bdelete! ' . l:nn
            call remove(g:nuake_list,l:ccc)
        endif
    endif
endfunction
