...
__fuzzybrow_dir_ignore="_build|elm-stuff|node_modules|.git|.svn|.hg|deps|ex_gdax"
...
fzf_browser() {
  local res key sel prev_dir query stored_query tmp_prompt tmp_file tmp_dir bck
  local initial_dir
  initial_dir="$(pwd)"
  ...
  while true ; do
    ...
    case "$key" in
      ...
      return)
        ...
        if [[ -d "$tmp_file" ]]; then
            pushd "$tmp_file" > /dev/null 2>&1 || exit -1
        elif [[ "$bck" == ".." ]]; then
          echo "do nothing?"
          bck=""
        else
  ...
  if [[ "$__fuzzybrowse_recursive" == 1 ]]; then
    ...
    while read -r line ; do
      if [[ -d "$line" ]]; then
        ...
        printf "\e[36m$line/\e[0m $(cd "$line" && fdfind --max-depth 1 --type f |head -9 | grep -v -i "$__fuzzybrow_file_ignore_pat" | tr "\\n" "|" | sed 's/|/\\\e[36m | \\\e[0m/g')\n"
      fi
    done
  fi
}
...
__fuzzybrowse_file_source(){
  ...
  if [[ "$__fuzzybrowse_show_hidden" == 1 ]]; then
    ...
    fdfind --max-depth "$max_dep" --type f -H
  else

    fdfind  --max-depth "$max_dep" --type f
...
__fuzzybrowse_dir_source(){
  ...
  if [[ "$__fuzzybrowse_show_hidden" == 1 ]]; then
    ...
    fdfind --max-depth "$max_dep" -H  | __fuzzybrow_populate_dir_list
  else
    ...
    fdfind --max-depth "$max_dep"   | __fuzzybrow_populate_dir_list
  fi
}
...
__fuzzybrowse_all_source(){
  if [[ "$__fuzzybrowse_show_hidden" == 1 ]]; then
    ...
    fdfind -H | __fuzzybrow_populate_dir_list
  else
    ...
    fdfind  | __fuzzybrow_populate_dir_list
  fi
}
